...
(ns gurps.pages.character.spells.groups
  (:require ["@react-navigation/native" :as rnn]
            ...
            [gurps.widgets.base :refer [view text button flat-list]]
            [re-frame.core :as rf]))

(defn- row
  [item expanded-colleges spells nav]
  (let [item-txt   (:name item)
    ...
    (r/as-element
     ...
     [:> button {:key (str item-txt "-btn")
                 ...
                 :onPress #(-> nav (.navigate (i18n/label :t/spell-details) #js {:id item-txt}))}
...
(defn spell-groups-page
  ...
  (let [nav (rnn/useNavigation)
        expanded-sections (some-> (rf/subscribe [:college-list/expanded]) deref)
      ...
      {:data sections
       ...
       (fn [item-info-js]
         (let [item-info (->clj item-info-js :keywordize-keys true)
               {data :item} item-info]
           (if (string? data)
             (section-header {:title data} expanded-sections)
             (row data expanded-sections spells nav))))}]]))
