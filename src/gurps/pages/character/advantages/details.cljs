...
(defn advantage-details-page
  ...
  (let [nav        (rnn/useNavigation)
        id         (-> props ->clj :route :params :id str->key)
    ...
    [:> view {:style (tw "bg-white flex flex-col grow")}
     ...
     (when (not (id advantages))
       [:> view {:style (tw "absolute bottom-4 right-4")}
        [add-button {:on-click #(do (rf/dispatch [(keyword type :add) id (id by-name)])
                                    (-> nav (.navigate (i18n/label (keyword :t (str "character-" type))))))}]])]))
